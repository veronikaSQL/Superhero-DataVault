...
,MD5(ARRAY_TO_STRING(ARRAY_CONSTRUCT(
     UPPER(TRIM(IFNULL(HI.NAME,'')))
    ,UPPER(TRIM(IFNULL(HI.GENDER,'')))
    ,UPPER(TRIM(IFNULL(HI.EYE_COLOR,'')))
    ,UPPER(TRIM(IFNULL(HI.RACE,'')))
    ,UPPER(TRIM(IFNULL(HI.HAIR_COLOR,'')))
    ,UPPER(TRIM(IFNULL(HI.HEIGHT,'')))
    ,UPPER(TRIM(IFNULL(HI.WEIGHT,'')))
    ,UPPER(TRIM(IFNULL(HI.SKIN_COLOR,'')))
           ),'~')) AS RECORD_HASH_PII 
,HI.PUBLISHER 
,HI.ALIGNMENT
...
select name, *
from data_vault.staging.stg_superhero_set;
...
INSERT INTO DATA_VAULT.SUPERHERO.SAT_BEING_SUPERHERO_SET_BII
SELECT 
    SAT_BEING_SUPERHERO_SET_KEY as SAT_BEING_SUPERHERO_SET_BII,
    HUB_BEING_KEY,
    NAME,
    GENDER,
    EYE_COLOR,
    ...
    WEIGHT,
    SKIN_COLOR,
    RECORD_HASH_PII AS RECORD_HASH,
    LOAD_DATE
FROM DATA_VAULT.STAGING.STG_SUPERHERO_SET STG
    WHERE NOT EXISTS 
    (SELECT * FROM DATA_VAULT.SUPERHERO.SAT_BEING_SUPERHERO_SET_BII SB WHERE STG.HUB_BEING_KEY = SB.HUB_BEING_KEY 
     and STG.RECORD_HASH_PII = SB.RECORD_HASH)
;
